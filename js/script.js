...
const totalInspected = plan.sampleSize; // Force to exactly required
const correctedPcsPerBox = Math.floor(plan.sampleSize / boxesToOpen);
const remainder = plan.sampleSize % boxesToOpen;
const adjustedText = boxesToOpen + ' boxes will be opened. ' +
  'Inspect ' + correctedPcsPerBox + ' pcs per box' +
  (remainder > 0 ? ' (and add ' + remainder + ' extra pcs to final box)' : '');
...
samplingInstructions = `
  ...
  <ul>
    <li>Randomly select and open <strong>${boxesToOpen}</strong> box(es) (from ${numBoxesVal} total).</li>
    <li>From each opened box, inspect <strong>${pcsPerOpenedBox}</strong> piece(s)</li>
    ${remainder > 0 ? `<li>From the last box, inspect only <strong>${remainder}</strong> piece(s)</li>` : ''}
  </ul>
