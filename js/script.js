...
const fractionToOpen = 0.25; // Always use ¼ of total boxes
const boxesToOpen = Math.min(Math.ceil(numBoxesVal * fractionToOpen), numBoxesVal);
...
samplingInstructions = `
  ...
  <ul>
    <li>Randomly select and open <strong>${boxesToOpen}</strong> box(es) (approximately 25% of ${numBoxesVal} total).</li>
    <li>From each opened box, inspect <strong>${pcsPerOpenedBox}</strong> piece(s).</li>
  </ul>
  <p><small>(Total pieces inspected: ${totalInspected}${totalInspected > plan.sampleSize ? ', slightly exceeding required sample size of ' + plan.sampleSize : ''})</small></p>`;
